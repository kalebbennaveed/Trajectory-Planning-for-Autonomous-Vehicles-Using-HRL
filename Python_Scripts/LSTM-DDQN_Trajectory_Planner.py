...
class VehiclePIDController():
    ...
    def __init__(self, vehicle, args_lateral=None, args_longitudinal=None):
        ...
        """
        if not args_lateral:
            args_lateral = {'K_P': 0.7, 'K_D': 0.005, 'K_I': 0.001}
        if not args_longitudinal:
            args_longitudinal = {'K_P': 0.7, 'K_D': 0.05, 'K_I': 0.01}
...
class PIDLongitudinalController():
    ...
    def __init__(self, vehicle, K_P=0.7, K_D=0.05, K_I=0.0, dt=0.03):
        """
        :param vehicle: actor to apply to local planner logic onto
...
class PIDLateralController():
    ...
    def __init__(self, vehicle, K_P=0.7, K_D=0.005, K_I=0.0, dt=0.03):
        """
        :param vehicle: actor to apply to local planner logic onto
        :param K_P: Proportional term
        :param K_D: Differential term
...
# Model Functions
# ---- DQN model (Yes) 
# ---- no double strcuture(DDQN)
# ---- no dueling structure(Dueling-DDQN)
# ---- no soft updates (1 - lambda) * weights
# ---- no hindsight experice replay
# ---- Time distributed layer and Lomg Short Term memory
# ======================================================================================================================================

# For LSTM layer. it is fed with 1 sample of 3 time steps and shape is 14 for state and 15 for next state 
# Input to LSTM Layer (1, 3, 14)
# Things to consider:
#   A) Alway have minimum 3 steps in each episode
#   B)
